...
class Sleep:
    ...
    def draw(boy):
        if boy.face_dir == -1:
            ...
            draw_rectangle(*boy.get_bb3())  # 튜플을 풀어헤쳐서 인자로 전달한다.

        else:
            boy.image.clip_composite_draw(int(boy.frame) * 100, 300, 100, 100,
                                          3.141592 / 2, '', boy.x - 25, boy.y - 25, 100, 100)
            draw_rectangle(*boy.get_bb2())
...
class Boy:
    ...
    def get_bb2(self):
        return self.x - 70, self.y - 40, self.x + 10, self.y + 0

    def get_bb3(self):
        return self.x - 10, self.y - 40, self.x + 70, self.y + 0
